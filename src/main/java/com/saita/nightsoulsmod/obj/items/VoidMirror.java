...
public class VoidMirror extends Item {

	public VoidMirror(Properties properties) {
		super(properties);
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
    	...
    	else if(playerIn.world.getDimensionKey().getLocation().getPath() == "the_end")
    	{
    		playerIn.teleportKeepLoaded(0, 128, 0);
    		worldIn.playSound(playerIn, playerIn.getPosition(), SoundInit.MAGIC_MIRROR.get(), SoundCategory.MASTER, 1.0F, 1.0F);
    		
    		 if(!playerIn.isCreative())
	     	  {
    			   playerIn.addPotionEffect(new EffectInstance(Effects.BLINDNESS, 40, 0));
	     		   playerIn.addPotionEffect(new EffectInstance(Effects.NAUSEA, 200, 0));
	     		   playerIn.addPotionEffect(new EffectInstance(Effects.SLOW_FALLING, 500, 0));
	     		   playerIn.getCooldownTracker().setCooldown(this, 300);
	     	  }
