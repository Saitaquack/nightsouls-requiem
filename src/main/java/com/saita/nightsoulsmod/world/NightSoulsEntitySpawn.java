...
public class NightSoulsEntitySpawn {
	...
	public static void onEntitySpawn(final BiomeLoadingEvent event) {
		  ...
		  if(!(event.getCategory().equals(Biome.Category.NETHER) || event.getCategory().equals(Biome.Category.THEEND) || event.getCategory().equals(Biome.Category.MUSHROOM)))
		  {
			  ...
			  addEntity(event.getSpawns(), NightSoulsEntityTypes.WEREWOLF.get(), 4, 1, 3);
			  ...
			  addEntity(event.getSpawns(), NightSoulsEntityTypes.PARTYGOER.get(), 1, 1, 3);
			  addEntity(event.getSpawns(), NightSoulsEntityTypes.WINDIGO.get(), 1, 1, 2);
		  ...
		  /* Make Hellborns and Hellfire Servants spawn in modded nether biomes */
		  
		  if((event.getCategory().equals(Biome.Category.NETHER)) && 
			  (!event.getName().equals(Biomes.NETHER_WASTES.getLocation()) || 
			   !event.getName().equals(Biomes.CRIMSON_FOREST.getLocation()) || 
			   !event.getName().equals(Biomes.WARPED_FOREST.getLocation()) || 
			   !event.getName().equals(Biomes.SOUL_SAND_VALLEY.getLocation()) || 
			   !event.getName().equals(Biomes.BASALT_DELTAS.getLocation())))
			{
			  addEntity(event.getSpawns(), NightSoulsEntityTypes.HELLBORN.get(), 3, 1, 4);
		  ...
		  if(event.getCategory().equals(Biome.Category.THEEND))
		    {
			  ...
			  addEntity(event.getSpawns(), NightSoulsEntityTypes.ALTERNATE.get(), 1, 0, 1);
